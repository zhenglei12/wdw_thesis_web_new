<template>
  <a-modal
    ...
    @ok="submit"
  >
    <div class="group-container" v-for="(group, index) in allPermission.items || []" :key="'group-' + index">
      <div class="group-header">
        <a-checkbox :indeterminate="group.anyChecked" v-model="group.checked" :disabled="group.disabled">{{
          group.zhName
        }}</a-checkbox>
      </div>
    ...
    <template slot="footer">
      <span style="margin-right: 16px">已选 {{ allPermission.checkedCount }} 项</span>
...
export default {
  ...
  methods: {
    ...
    submit() {
      ...
      RoleApi.allot({
        id: this.R.id,
        permissions: this.allPermission.getChecked(null, (item) => item.data && item.data.name),
      })
        .then((res) => {
